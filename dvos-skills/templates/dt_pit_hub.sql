-- Dynamic Table PIT — Hub + 3 Satellites
...
CREATE OR REPLACE DYNAMIC TABLE <database>.<queryassistance_schema>.dt_pit_<hub_name>_<cadence>
...
, hub AS (
  SELECT <business_key_name>
       , dv_hashkey_hub_<hub_name>
       , dv_applied_timestamp AS hub_begin_date
  FROM <datavault_schema>.hub_<hub_name>
)
, stalk AS (
  SELECT <business_key_name>
       , dv_hashkey_hub_<hub_name>
  ...
  FROM as_of
  INNER JOIN hub ON 1=1
)

SELECT st.*

-- sat1: hashkey forward-fill with ghost fallback
, COALESCE(s1.dv_hashkey_hub_<hub_name>
    , LAG(s1.dv_hashkey_hub_<hub_name>) IGNORE NULLS OVER
      (PARTITION BY st.dv_hashkey_hub_<hub_name> ORDER BY st.snapshotdate)
    , TO_BINARY(REPEAT(0, 20)))
    AS <sat1_name>_dv_hashkey_hub_<hub_name>

-- sat2: hashkey forward-fill
, COALESCE(s2.dv_hashkey_hub_<hub_name>
    , LAG(s2.dv_hashkey_hub_<hub_name>) IGNORE NULLS OVER
      (PARTITION BY st.dv_hashkey_hub_<hub_name> ORDER BY st.snapshotdate)
    , TO_BINARY(REPEAT(0, 20)))
    AS <sat2_name>_dv_hashkey_hub_<hub_name>

-- sat3: hashkey forward-fill
, COALESCE(s3.dv_hashkey_hub_<hub_name>
    , LAG(s3.dv_hashkey_hub_<hub_name>) IGNORE NULLS OVER
      (PARTITION BY st.dv_hashkey_hub_<hub_name> ORDER BY st.snapshotdate)
    , TO_BINARY(REPEAT(0, 20)))
    AS <sat3_name>_dv_hashkey_hub_<hub_name>
...
LEFT JOIN <datavault_schema>.<sat1_name> s1
  ON st.dv_hashkey_hub_<hub_name> = s1.dv_hashkey_hub_<hub_name>
 AND st.snapshotdate = s1.dv_applied_timestamp
LEFT JOIN <datavault_schema>.<sat2_name> s2
  ON st.dv_hashkey_hub_<hub_name> = s2.dv_hashkey_hub_<hub_name>
 AND st.snapshotdate = s2.dv_applied_timestamp
LEFT JOIN <datavault_schema>.<sat3_name> s3
  ON st.dv_hashkey_hub_<hub_name> = s3.dv_hashkey_hub_<hub_name>
